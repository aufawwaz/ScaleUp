        ...
        <table class="min-w-full rounded-xl overflow-clip">
          ...
          <tbody>
            ...
            <tr onclick="window.location='{{ route('contact.show', $contact->id) }}'"
              class="hover:bg-gray-50 transition cursor-pointer border-b border-gray-100">
              <td class="py-4 px-3 font-normal text-xs">
                {{ $loop->iteration + ($contacts->currentPage() - 1) * $contacts->perPage() }}
              </td>       
              <td class="py-4 px-3 font-normal items-center text-xs flex gap-2">
                @if ($contact->image_kontak)
                  <img src="{{ asset('storage/' . $contact->image_kontak) }}"
                      alt="{{ $contact->nama_kontak }}"
                      class="w-8 aspect-square object-cover rounded-full transition duration-300 group-hover:scale-105" />
                @else
              ...
              <td class="py-4 px-3 font-normal text-xs">{{ Str::limit($contact->nomor_handphone, 20) }}</td>
              <td class="py-4 px-3 font-normal text-xs">{{ Str::limit($contact->email_kontak, 30) }}</td>
              <td class="py-4 px-3 font-normal text-xs">{{ Str::limit($contact->alamat_kontak, 30) }}</td>
              <td class="py-4 px-3 font-normal text-xs">{{ $contact->jumlah_transaksi }}</td>
              <td class="py-4 px-3 font-normal text-xs flex w-fit gap-1">
                ...
                ></x-custom-button>
                <x-custom-button href="{{ route('contact.destroy', $contact->id) }}" outline="true" color="danger" size="sm"
