       ...
       <form action="{{ route('contact.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="bg-white rounded-2xl shadow p-6 flex flex-col gap-4">
            ...
            <x-custom-input-form label="Nama Kontak" name="nama_kontak" required="true" placeholder="Masukkan Nama Kontak" />
          </div>
          
          <!-- Kolom Kanan: Detail Kontak (Opsional) -->
          <div class="bg-white rounded-2xl shadow p-6 flex flex-col gap-4">
            <label class="font-semibold mb-2">DETAIL KONTAK</label>
            <x-custom-input-form label="Nomor Handphone" name="nomor_handphone" placeholder="Masukkan Nomor Handphone" />
            <x-custom-input-form label="Email" name="email_kontak" placeholder="Masukkan Email Kontak" />
            <x-custom-input-form label="Alamat" name="alamat_kontak" type="textarea" placeholder="Masukkan Alamat Kontak" />
          </div>
        </div>
