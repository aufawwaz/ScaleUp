...
<body style="overflow-y: auto;" class="min-h-dvh bg-white font-poppins">
    ...
    <div class="flex w-full min-h-dvh justify-center items-center">
        ...
        <div data-aos="fade-right" class="inset-shadow-sm h-[580px] shadow-2xl overflow-clip bg-primary-gradient flex rounded-[20px] items-center justify-between w-8/12">
            {{-- on board --}}
            <div class="relative w-full h-[28rem] overflow-hidden text-white md:ml-auto mb-4 md:mb-0 md:mr-0 flex flex-col justify-center gap-2" style="transition: opacity 0.3s ease-in-out, transform 0.3s ease-in-out;">
                <div data-aos="fade-up" data-aos-delay="300" id="auto-scroll" class="flex h-fit transition-transform duration-700 ease-in-out mb-4">
                    ...
                    <div class="w-full h-full px-20 flex-shrink-0 flex flex-col items-center justify-center ">
                        ...
                        <h3 class="text-xl font-bold text-center mb-3">Kelola Transaksi Secara Real-Time</h3>
                        <p class="text-xs text-center">Pantau saldo dan transaksi bisnis secara instan untuk pengelolaan yang lebih efisien</p>
                    </div>
                </div>
                <div data-aos="zoom-in" data-aos-delay="500" class="w-full h-3 flex gap-1 justify-center items-center">
            ...
            {{-- form login --}}
            <div class="container relative h-full bg-white px-[3rem] py-[1rem] w-3/4" style="transition: opacity 0.5s ease-in-out, transform 0.5s ease-in-out;">
                <div data-aos="fade-right" data-aos-delay="300" class="z-10 flex mt-5 mb-2 items-center justify-start">
                    <img src="{{ asset('asset\logo.svg') }}" alt="" class="w-8">
                    <p class="text-primary text-2xl transform -translate-x-1.25">cale<b>Up</b></p>
                </div>
                <div data-aos="fade-up" data-aos-delay="500">             
                    <h2 class="z-10 text-dark text-2xl text-center font-bold py-[1rem] mb-2">Login</h2>
                    <a class="z-10" href="{{ route('google.redirect') }}">
                        <button
                        onclick="" 
                        class="rounded-[12px] border-1 border-gray h-[44px] w-full flex flex-row justify-center items-center gap-1 cursor-pointer"
                        >
                            <img src="/asset/ic_google.svg" alt="" style="width: 12px;">
                            <p class="text-dark text-xs font-bold mb-0.5">Sign in with Google</p>
                        </button>
                    </a>
                    
                    <div class="z-10 flex items-center justify-center gap-x-[8px] h-[32px]">
                        <hr class="w-[30%] border-gray">
                        <p class="text-gray text-xs mb-1">or</p>
                        <hr class="w-[30%] border-gray">
                    </div>
                    
                    <form action="{{ route('login.process') }}" method="POST" class="z-10 flex flex-col gap-[16px] mb-[3rem]">
                        @csrf
                        <x-input-form autocomplete="off" type="text" name="email" icon="/asset/ic_email.svg" placeholder="Email"/>
                        ...
                        <div data-aos="zoom-in" data-aos-delay="800" class="w-full flex flex-col gap-[16px]">
                            <div class="flex gap-2 items-center mt-4">
                                <input type="checkbox" name="remember_me" class="cursor-pointer accent-gray checked:accent-primary h-[24px]">
                                <p class="text-xs mb-0.5">Remember Me</p>
                            </div>

                            <x-custom-button type="submit" size="md"><span class="text-sm font-bold">Login</span></x-custom-button>
                            
                            <p class="text-xs">
                                Don't have an account?
                                <a href="{{ route("register") }}" class="text-xs text-primary">Create Account</a>
                            </p>
                        </div>
                    </form>
                </div>
