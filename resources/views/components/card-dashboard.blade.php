...
<div class="h-[154px] flex flex-col justify-between p-[1rem] rounded-2xl bg-white text-dark shadow w-full" data-aos="{{ $aos }}">
    <p class="text-sm font-medium opacity-60">{{ $title }}</p>
    <div class="flex gap-[1rem] items-center">
        {!! $icon !!}
        <p class="text-xl font-semibold">{{ $value }}</p>
    </div>
    <a href="{{ $buttonUrl }}" class="flex justify-between items-center font-medium bg-primary/10 hover:bg-primary hover:text-white hover:scale-102 text-primary p-2 px-3 text-xs rounded-xl transition">
        <p>{{ $buttonLabel }}</p>
