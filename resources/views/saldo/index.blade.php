...
<x-layout   
  ...
  title="Saldo"  
>
  <div x-data="{ $store.modal.show: {{ ($errors->any()) ? 'true' : 'false' }} }">
    <main class="main-container p-4 overflow-hidden cursor-default">
      
      {{-- total saldonya --}}
      @if(!$isNoData)
        <div
          class="flex flex-col shadow-sm rounded-2xl py-4 px-4 gap-1 cursor-pointer transition-all select-none"
          @click="$store.saldoActive.set(-1, '', ''); fetch('/saldo/fetch/-1')
            .then(r => r.json())
            .then(d => $store.transactions.set(d))
            .catch(console.error)"
...
<script>
  ...
  document.addEventListener('alpine:init', () => {
    ...
    fetch('/saldo/fetch/-1')
      .then(res => res.json())
      .then(data => Alpine.store('transactions').set(data))
      .catch(console.error)
  });
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    Alpine.effect(() => {
      ...
      if (Array.isArray(newData) && newData.length) {
        // sort per-tanggal
        let sorted = [...newData].sort((a, b) => new Date(a.tanggal) - new Date(b.tanggal));
        let saldoHarian = [];
        let labels = [];
        let saldo = saldoAwal;

        // data akan dijumlahkan dengan saldo
        sorted.forEach(item => {
